...
def vis_obj(box, img, calib, hw, color_tmp=None, str_vis=None, thickness=4, id_hl=None, err_type=None):
    ...
    if err_type is not None:
        ...
        cv2.rectangle(img, (pos_x, pos_y - text_h - 5), (pos_x + text_w, pos_y + 5), (255, 255, 255),
        ...
        if depth >= 2:  # check in front of camera
            ...
            if color_type == 'trk':
                ...
                if id_hl is not None and obj.ID in id_hl:
                    ...
                    color_id = obj.ID * 9  # some magic number to scale up the id so that nearby ID does not have
                    # similar color
