...
class trackingEvaluation(object):
    ...
    def __init__(self, t_sha, gt_path="./scripts/KITTI", max_truncation=0, min_height=25, max_occlusion=2, \
        ...
        self.n_gts = []  # number of ground truth detections minus ignored false negatives and true positives PER
        # SEQUENCE
        ...
        self.n_igttr = 0  # number of ignored ground truth detections where the corresponding associated tracker
    ...
    def getThresholds(self, scores, num_gt, num_sample_pts=num_sample_pts):
        # based on score of true positive to discretize the recall not necessarily have data on all points due to not
        # fully recall the results, all the results point has zero precision compute the recall based on the gt
        # positives
    ...
    def compute3rdPartyMetrics(self, threshold=-10000, recall_thres=1.0):
        ...
        if self.n_gt == 0:
            ...
            self.sMOTA = min(1, max(0,
                                    1 - (self.fn + self.fp + self.id_switches - (1 - recall_thres) * self.n_gt) / float(
    ...
    def createSummary_details(self):
        ...
        # summary += "\n" summary += self.printEntry("Ignored Tracker Objects with Associated Ignored Ground Truth
        # Objects", self.n_igttr) + "\n"
        summary += "=" * 80
...
class stat:
    ...
    def plot_over_recall(self, data_list, title, y_name, save_path):
        ...
        if y_name in ['MOTA', 'F1']:
            ...
            plt.text(self.recall_list[max_ind] - 0.05, data_list[max_ind] + 0.03, '%.2f' % (data_list[max_ind] * 100),
                     fontsize=20)
    ...
    def plot(self):
        ...
        self.plot_over_recall(self.sMOTA_list, 'sMOTA - Recall Curve', 'sMOTA',
...
def evaluate(result_sha, mail, eval_3diou, eval_2diou, thres):
    ...
    for c in ("cyclist", "pedestrian", "car"):
        ...
        if len(e.groundtruth) != len(e.tracker):
            mail.msg("The uploaded data does not provide results for every sequence: %d vs %d" % (
